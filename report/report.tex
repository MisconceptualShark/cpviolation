...
To generate the masses of the up-type quarks, the hermitian cojugate term is used, where now a second Higgs doublet (not dependent of the first, but its charge conjugate) is introduced as
\begin{align}
    \label{eq:highc}
...
Through the weak force, there are two ways for CP violating interactions to occur: through complex Yukawa couplings in the CKM matrix; and through complex Higgs parameters, such as those in the 2 Higgs Doublet Model to be discussed in section \ref{subsec:2hdm}.
...
The charged Higgs particles can mediate weak charged currents in the same way as $W^{\pm}$, and the additional contribution of $H^{\pm}$ to many weak-mediated decays could correct the Standard Model to correctly predict these decay paths \cite{s}. 
...
However, here it can be scene that the global constraint region is limited to within the region of parameter space shown, so both upper and lower limits for the 2HDM parameters can be found.
Here, it is found as
\begin{align}
    \label{eq:const}
