...
These new variables are: the masses of the neutral $h^0,H^0$, the mixing angle $\alpha$, and the 2HDM potential parameter $M=\frac{m_{12}^2}{\sin\beta\cos\beta}$. 
...
When working in the 2HDM, it is usually assumed that we are working in the \emph{alignment limit}, where the lighter neutral Higgs $h^0$ is taken to be the SM Higgs, and therefore the mixing angles are aligned such that $\cos(\beta-\alpha) \approx 0$. 
...
The extent of the constraint on $\tan\beta$ does depend on $m_{H^+}$ as seen in Figure \ref{fig:bmu}.
When added to the global constraints, this will restrict larger $\tan\beta$ values depending on $m_{H^0}$, though we see smaller $\tan\beta$ are still allowed as $m_{H^+}\to\infty$.
Testing across the range of values for $m_{H^0}$, we find that using $m_{H^0}=1.5\,$TeV minimises $\chi^2$ and maximises the p-value of our fit; see Section ref and Table \ref{tab:pval} for details of this fit.
...
We therefore move forward with two approaches to work through this discrepancy: we continue to fit all observables at the $2\sigma$ level, excluding $\mathcal{R}(D^*)$ as an anomaly as it is in the Standard Model; and we also scan to higher $\sigma$, as $\mathcal{R}(D^*)$ may then agree with the fit to this level, suggesting further new physics to explain its deviation from the 2HDM.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.49\textwidth]{rd_both5sig2.png}
    \caption{\label{fig:rd5sig}Constraint on ($m_{H^+},\tan\beta$) space from $\mathcal{R}(D)$ and $\mathcal{R}(D^*)$ from a 5$\sigma$ scan. 
    The turquoise region shows what is not excluded for the joint fit of these two observables from the $5\sigma$ scan, and the blue line indicates the outline of the 95\% CL region.}
\end{figure}
In Figure \ref{fig:rd5sig}, it is shown that the joint constraint from $\mathcal{R}(D)$ and $\mathcal{R}(D^*)$ aligns well with that of the leptonic decays in Figure \ref{fig:indies} when we now scan at $5\sigma$. 
Something else here. 
...
Moving forward, we consider both the exact wrong sign limit from Eq. \ref{eq:wrongsign}, and the use of numerical constraints on $\cos(\beta-\alpha)$ provided from collaboration of the fit with \cite{oliver}.
...
These have not been the focus of this study, but we collaborate with \cite{james} for their work on the oblique parameters in the 2HDM (working from the expressions given in \cite{oblique}), and add these to the global 2HDM fit. 
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\textwidth]{../global/threedimfit.png}
    \caption{\label{fig:threed}Constraints in ($m_{H^+},m_{A^0},\tan\beta$) space from $B\to\mu^+\mu^-$ and the Oblique parameters, where we assume $m_{A^0}\approx m_{H^0}$, and have scanned to $2\sigma$.}
\end{figure}
In Figure \ref{fig:threed}, we see that there is an approximate linear relationship in between $m_{A^0}$ and $m_{H^+}$, particularly for higher values. 
The value of $m_{H^+}$ is therefore tightly constrained to be of a similar mass to $m_{H^0}$ and $m_{A^0}$. 
\footnote{In \cite{james}, it is found that there can be some fine-tuned solutions away from this confinement, however, these are for choices of $\theta$ away from either the alignment or wrong sign limits which we consider here.}
We find that for increased values of $m_{H^0}\approx m_{A^0}\approx m_{H^+}$, the value of $\chi^2$ for our fit decreases. 
This is, however, moving towards the decoupling limit of the 2HDM and therefore recovering the Standard Model solutions, so it yields little information for testing a realistic 2HDM. 
We choose to work with $m_{H^0}=m_{A^0}=1.5\,$TeV as chosen earlier and test the global fit using these fixed parameters. 
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.49\textwidth]{../global/global_test.png}
    \includegraphics[width=0.49\textwidth]{../global/global_test1.png}
    \includegraphics[width=0.49\textwidth]{../global/global_test3.png}
    \includegraphics[width=0.49\textwidth]{../global/global_test4.png}
\end{figure}

\section{Conclusion}
Something about checking calculations with Tom \cite{tom}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{cc|C{1.8cm}|C{2.1cm}|C{1.8cm}|C{2.1cm}}
        ...
        \hline\hline
    \end{tabular}
    \caption{\label{tab:pval}Constraints and statistics for various global fits. `Flavour' denotes all of the flavour observables discussed throughout, disregarding $R(D^*)$ for now; `Oblique' denotes adding the three oblique parameters from James' work, S,T,U. 
