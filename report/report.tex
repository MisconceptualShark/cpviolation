...
\abstract{
    In this study, we test the Two Higgs Doublet Model (2HDM) of Type II as an extension of the Standard Model (SM) using indicative flavour observables, such as some leptonic and semileptonic decays of $B$ and $D$ mesons, $B\bar{B}$ mixing, the $b\to s\gamma$ radiative decay.
...
\end{align}
where $x_{ij} = m_i^2/m_j^2$, and $S_{xy}$ describes the internal bosonic lines for the two bosons $x,y \in H^\pm,W^\pm$ \cite{frank}.
...
Higher values than this would be more even favourable, however, we choose not to work at such high scales as discussed in Section \ref{sec:discuss}.
\begin{table}[ht]
    \centering
...
This is likely due to the upper and lower regions the oblique parameters have in this limit.
...
However, we do see in $\chi^2_\nu$ and the p-values that although the errors do not yet fully dominate, they are significant enough that we begin to lose the significance in our results at this level.
...
The maximum allowed value for $\tan\beta$ is seen to increase with larger $m_{H^0}$, as shown in Figure \ref{fig:threed}, and so, this is only indicative of the limitations for $\tan\beta$ for the more finite masses we consider.
...
We have found that we can exclude this model from $2\sigma$ error signifance to 95\% CL in the wrong sign limit, and 85\% CL in the alignment - these were found both in a two-dimensional fit with fixed $m_{H^0}=m_{A^0}$, and a three-dimensional fit varying $m_{H^0}$.
The model cannot be excluded with large confidence at the $3\sigma$ error significance.
The most significant effect towards to poor fitting of this model is the semileptonic ratio $\mathcal{R}(D^*)$, which remains anomalous in the 2HDM to $3\sigma$. 
