...
As in many parts of quantum mechanics, it can be useful to define the fermion field $\psi$ into an upper and lower part, such that $\psi$ can be written as a linear combination of componenet states. 
...
Scanning in ($m_{H^+},\tan\beta$) space, as for other observables, for fixed values of $M$ and $m_{H^0}$ across a range based on the work in \cite{trilin} and the direct searches for $H^0$ in \cite{h0search}, we find that $M$ bears little significance on the shape of the scan, particularly in regions of overlap with other observables. 
...
\end{figure}
As we had previously, we see that there is no specific constraint on $m_{H^+}$ as we move towards $\infty$, and these processes by themselves do not give a minimum value of $m_{H^+}$. 
We do however now see more significant bounds on $\tan\beta$ than we had previously. 
The extent of the constraints on $\tan\beta$ does depend on $m_{H^+}$ as seen in Figure \ref{fig:bmu}; for higher $m_{H^0}$, $\tan\beta$ can be higher too.
...
In \cite{desc}, a 2HDM parameterisation of $\mathcal{R}(D)$ was discussed and was said to align similarly with the leptonics constraints, however, they did not discuss $\mathcal{R}(D^*)$. 
...
\begin{figure}[ht]
    ...
    The black lines for each region outlines the 95\% CL region and the blue line outlines the $1\sigma$ confidence region.}
\end{figure}
