...
\end{equation}
combining all constituent gauge groups, with $g_{1,2,3}$ the gauge coupling of U(1)$_Y$, SU(2)$_L$, and SU(3)$_c$ respectively where the gauge field(s) and generator(s) of each group are in the term of their coupling.   
...
\begin{equation}
    ...
    [T^a,T^b] = if^{abc}T^c,
\end{equation}
where $f^{abc}$ are the structure constants which define each SU(N) group. 
...
\end{equation}
Through substituting the above into \eqref{eq:higgslag}, the gauge fields of the real electroweak bosons $W^{\pm},Z,\gamma$ can be found as linear combinations of $W^i_\mu$ and $B_\mu$ with their mass terms now generated in a gauge invariant form:
\begin{align}
    \label{eq:gagmix}
...
\end{align}
It can be said that three of component scalar fields in \eqref{eq:doubscal} were "eaten" to form the gauge boson masses, and the fourth field goes on to become the Higgs phenomenom.\\
...
The hermitian conjugate term is required so both up-type quarks, and down-type quarks and charged leptons gain mass. 
...
\end{align}
where $i$ represents whichever fermion is in question. 
...
In \eqref{eq:yuk} and \eqref{eq:yukhc}, we have formed non-diagonal mass matrices. 
...
By convention, it is chosen to diagonalise the up-type quarks, and then rotate the down-type quarks between their weak eigenstates and their mass eigenstates. 
...
The strengths of quark coupling is stored in elements of the CKM matrix as
\begin{align}
    \label{eq:ckmub}
...
There are many parameterisations that can be used for this, though the two most common are the \emph{standard parameterisation} and the \emph{Wolfenstein parameterisation}.
...
\hspace{-5pt}The presence of a complex phase allows for CP-violating processes to take place in the Standard Model: the CKM matrix's Hermitian conjugate would will give the complex conjugated phase, yielding different physics over CP conjugation for complex coupling elements. 
...
\label{subsec:2hdm}
Previously in the Standard Model Higgs mechanism, to generate the masses of the weak gauge bosons and fermions, a single Higgs SU(2) doublet was introduced of complex scalar fields, as in \eqref{eq:doubscal}.
To form masses for both up- and down-type quarks and charged leptons, we introduced the hermitian conjugate doublet in \eqref{eq:highc}, otherwise the Higgs field would only give mass to the down-type quarks. 
